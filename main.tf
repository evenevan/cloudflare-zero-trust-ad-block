...
locals {
  hosts_invalid   = ["127.0.0.1  localhost", "::1  localhost"]
  hosts           = split("\n", data.curl.get_hosts.response)
  hosts_formatted = [for host in local.hosts : split(" ", host)[1] if host != "" && !startswith(host, "#") && !contains(local.hosts_invalid, host)]
